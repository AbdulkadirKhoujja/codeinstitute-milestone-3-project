...
| 1.3 Database-backed data manipulation | Tested Post CRUD, profiles, feeds, search, filters, sorting, and pagination | 2 | Complete | `news/views.py`, `news/forms.py`, `accounts/views.py`, test packages |
...
| 1.5 Test procedures | Isolated automated tests and documented TDD cycles; later manual matrices remain | 1–4 | In progress | Test packages and `docs/testing.md` |
...
| 1.7 Python proficiency | Views, model forms, ORM filters, aggregation, permissions, validation, pagination, and useful return values | 2–3 | Complete | `accounts/`, `news/views.py`, `news/forms.py`, tests |
...
| 3.1 Create, locate, display, edit, and delete records | Full Post CRUD plus feeds, profiles, category filtering, search, sorting, pagination, ownership, and privacy | 2 | Complete | News/account views, forms, templates, and tests |
...
| M(vi) Fully documented testing | Phase 2 commands, coverage, representative failures, corrections, results, and limitation recorded | 2–4 | In progress | `docs/testing.md`; later formal evidence remains |
...
| M(x) Working CRUD | Complete authenticated owner-restricted Post create, read, update, and delete | 2 | Complete | Views, form, URLs, templates, and tests |
| M(xi) CRUD immediately reflected | Successful actions redirect to detail/feed destinations that show the change or removal with feedback | 2 | Complete | Integration tests and rendered pages |
...
| Original, fully functioning application | Core accounts, private drafts, owned posts, advanced discovery, and moderation-ready data are working; community actions remain | 2–3 | In progress | Application and automated tests |
...
| Clear front-end/back-end relationship | Forms, views, templates, ORM queries, permissions, messages, and relational records are connected and explained | 2–3 | Complete | Code, README architecture section, and tests |
...
| Defensive, secure behaviour | CSRF, authenticated mutations, private drafts, safe redirects, ownership, method restrictions, and environment secrets | 2–4 | In progress | Negative-path tests and future production security review |
...
- **Phase 3 planned:** Comment CRUD, public voting actions, custom JavaScript, external-news integration with caching/error handling, custom error pages, and community UX refinement.
