...
class ReleaseSettingsTests(SimpleTestCase):
        ...
        with patch.dict(os.environ, environment, clear=True):
            with patch("os.path.isfile", return_value=False):
                return runpy.run_path(
                    settings.BASE_DIR / "byteboard/settings.py"
                )
    ...
    def test_invalid_boolean_does_not_silently_enable_debug(self):
        with self.assertRaises(ImproperlyConfigured):
            self.load_settings(
                SECRET_KEY="test-only-placeholder", DEBUG="maybe")
    ...
    def test_postgresql_url_is_parsed_in_authoritative_settings(self):
        config = self.load_settings(
            ...
            DATABASE_URL=(
                "postgresql://member:placeholder@localhost:5432/byteboard"
            ),
        )
    ...
    def test_database_url_errors_do_not_echo_credentials(self):
        with self.assertRaisesMessage(
            ...
            self.load_settings(
                SECRET_KEY="test-only-placeholder",
                DATABASE_URL=(
                    "unknown://member:private-placeholder@host/database"
                ),
            )

    def test_collected_assets_have_a_production_serving_backend(self):
        config = self.load_settings(SECRET_KEY="test-only-placeholder")
        self.assertEqual(config["STATIC_ROOT"],
